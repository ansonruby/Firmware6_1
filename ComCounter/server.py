...
class WebsocketServer:
    ...
    def __init__(self, host, port, queues,
                 ws_cls, print_logs=None):
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.host = host
        self.port = port
        self.queues = queues
        self.ws_cls = ws_cls
        self.print_logs = print_logs if print_logs else lambda x: x

    def run(self):
        self.socket.bind((self.host, self.port))
        self.socket.listen(self.queues)
        while True:
            self.print_logs('[SERVER]= Waiting connection at ' +
                            str(self.host)+":"+str(self.port))
            conn, addr = self.socket.accept()
            self.print_logs('[SERVER]= Connected from '+str(addr))
