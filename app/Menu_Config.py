...
from lib.Lib_Requests_Server import *  # importar con los mismos nombres
...
def clickbut_Tiempo(number):
        ...
        #Tiempo Torniquete
        Set_File(CONF_TIEM_RELE, str(Tiempo_Torniquete))
...
def clickbut(number):   #lambda:clickbut(1)
        ...
        if number == '◄':
                ...
                if w is IP:
                    ...
                    #print "It's contrasena"
                    if len(operatorDNS) > 0:
                        operatorDNS=operatorDNS+str(number)
                        textinDNS.set(operatorDNS)
                        #sonidoLector(Tiempo_sonido)
                    elif number != 0:
                        operatorDNS=operatorDNS+str(number)
                        textinDNS.set(operatorDNS)
...
def ver_IP_Static():
        ...
        L_Ip_Static.place(bordermode=OUTSIDE, height=20, width=100, x= 115, y=2)
        L_Ip_Static_lista.place(bordermode=OUTSIDE, height=29, width=200, x=60, y=25)

        L_Ip.place(bordermode=OUTSIDE, height=20, width=100, y=55)
        IP.place(bordermode=OUTSIDE, height=30, width=200, x=90, y=55)

        L_Gat.place(bordermode=OUTSIDE, height=20, width=100, y=90)
        Gateway.place(bordermode=OUTSIDE, height=30, width=200, x=90, y=90)

        L_DNS.place(bordermode=OUTSIDE, height=20, width=100, y=125)
        DNS.place(bordermode=OUTSIDE, height=30, width=200, x=90, y=125)
        ...
        butespip.place(x=Ini_xip+(Disxip*0), y=Ini_yip+(Disyip*5))
        butSINESTATIC.place(x=Ini_xip+(Disxip*1)+40, y=Ini_yip+(Disyip*5))
...
def No_ver_IP_Static():
        ...
        but0.place_forget()
        butp.place_forget()
        butespip.place_forget()
        butSINESTATIC.place_forget()
...
def L_menu_inicio():
        ...
        #Progreso_Servidor.place_forget()
...
def verificar_IP_Static(): #esta función verifica que este correcto usuario y contraseña digitado por el usuario
    ...
    if (len(Ip1) == 0) or (len(Gateway1)==0) or (len(Tipo_red)==0):
        ...
        if (check_IP(Ip1) == False) or (check_IP(Gateway1) == False) :
                ...
                if (len(RED_DNS) == 0):
                    if Tipo_red == "Ethernet":
                            Modificar_Archivo1(1,1,0,0)
                    else:
                            Modificar_Archivo1(1,0,0,0)
                    commands.getoutput('sudo reboot')
                else:
                    ...
                    #s2 = RED_DNS.split(" ")
                    Bmadera_DNS=0
                    for linea in RED_DNS.split(' '):
                        #print len(linea)
                        if(len(linea) > 0):
                            if check_IP(linea) != True: Bmadera_DNS=1

                    if Bmadera_DNS == 1:
                        frame2 = Message(top, font='Arial', relief=RAISED, text='DNS NO valida.',padx=50,pady=50,width=100, bg='Red')
                        frame2.pack()
                    else:
                        if Tipo_red == "Ethernet":
                                Modificar_Archivo1(1,1,1,0)
                        else:
                                Modificar_Archivo1(1,0,1,0)
                        commands.getoutput('sudo reboot')
...
def verificar_wifi(): #esta función verifica que este correcto usuario y contraseña digitado por el usuario
    ...
    if (len(red) == 0) or (len(clave)==0):
        ...
        if BK_Red == 0:
            ...
            Nueva_wifi='\nnetwork={\n\tssid="'+red+'"\n\tpsk="'+clave+'"\n\tkey_mgmt=WPA-PSK\n}'
            #print (Nueva_wifi)
            Add_File(CONF_WIF_ETHE, Nueva_wifi)
            commands.getoutput('sudo reboot')
...
def Ver_Nuevo_Servidor():
        ...
        V_Nuevo_Servidor_Minusculas()
        textin6.set(Get_File(CONF_DOMI_SERVER))
...
def V_Nuevo_Servidor_Minusculas():
        ...
        Test_Nuevo_Servidor.place(x=Ini_x+100, y=Ini_y-42)
        #wifi
...
L_DNS = Label(tk, font='Arial', bg='Dark gray', text="DNS: ")
...
DNS=Entry(tk, font='Arial',textvar=textinDNS) # , show='*'se escoge encriptar con * la contraseña
...
butespip=Button(tk,padx=DXw+40,pady=DYw,bd=BD,bg='white',command=lambda:clickbut(' '),text=' ',font=Fuenteip)
butSINESTATIC=Button(tk,padx=DXw+2,pady=DYw+9,bd=BD,bg='white',command=SIN_IP_Static,text='SIN IP STATIC',font=Fuente_10)
...
#Progreso_Servidor=Entry(tk, font='Arial',textvar=textProgreso,state="disabled")#Entry(tk, font='Arial',textvar=textin3)
#contrasena=Entry(tk, font='Arial',textvar=textin4) # , show='*'se escoge encriptar con * la contraseña
